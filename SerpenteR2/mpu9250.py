...
# Blatantly pludered from https://github.com/adafruit/Adafruit_CircuitPython_MPU6050.git
# Rebuild helper from MPU 6050 to MPU 9250 here
#   Reference implementation for MPU9250
#   https://github.com/sparkfun/MPU-9250_Breakout/blob/master/Libraries/Arduino/src/SparkFunMPU9250-DMP.cpp
...
_MPU9250_DEFAULT_ADDRESS = 0x68  # MPU9250 default i2c address w/ AD0 low
_MPU9250_DEVICE_ID = 0x71  # The correct MPU9250_WHO_AM_I value

_MPU9250_SELF_TEST_X = 0x0D  # Self test factory calibrated values register SELF_TEST_X_ACCEL
_MPU9250_SELF_TEST_Y = 0x0E  # Self test factory calibrated values register SELF_TEST_Y_ACCEL
_MPU9250_SELF_TEST_Z = 0x0F  # Self test factory calibrated values register SELF_TEST_Z_ACCEL
_MPU9250_SELF_TEST_A = 0x10  # Self test factory calibrated values register ?!
_MPU9250_SMPLRT_DIV = 0x19  # sample rate divisor register SMPLRT_DIV
_MPU9250_CONFIG = 0x1A  # General configuration register
_MPU9250_GYRO_CONFIG = 0x1B  # Gyro specfic configuration register GYRO_CONFIG
_MPU9250_ACCEL_CONFIG = 0x1C  # Accelerometer specific configration register ACCEL_CONFIG
_MPU9250_INT_PIN_CONFIG = 0x37  # Interrupt pin configuration register
_MPU9250_ACCEL_OUT = 0x3B  # base address for sensor data reads ACCEL_XOUT_H
_MPU9250_TEMP_OUT = 0x41  # Temperature data high byte register
_MPU9250_GYRO_OUT = 0x43  # base address for sensor data reads
_MPU9250_SIG_PATH_RESET = 0x68  # register to reset sensor signal paths
_MPU9250_USER_CTRL = 0x6A  # FIFO and I2C Master control register
_MPU9250_PWR_MGMT_1 = 0x6B  # Primary power/sleep control register
_MPU9250_PWR_MGMT_2 = 0x6C  # Secondary power/sleep control register
_MPU9250_WHO_AM_I = 0x75  # Divice ID register
...
class MPU9250:
    """Driver for the MPU9250 6-DoF accelerometer and gyroscope.
    ...
    def __init__(self, i2c_bus, address=_MPU9250_DEFAULT_ADDRESS):
        self.i2c_device = i2c_device.I2CDevice(i2c_bus, address)

        if self._device_id != _MPU9250_DEVICE_ID:
            raise RuntimeError("Failed to find MPU9250 {0} -check your wiring!".format(self._device_id))
    ...
    _clock_source = RWBits(3, _MPU9250_PWR_MGMT_1, 0)
    _device_id = ROUnaryStruct(_MPU9250_WHO_AM_I, ">B")

    _reset = RWBit(_MPU9250_PWR_MGMT_1, 7, 1)
    _signal_path_reset = RWBits(3, _MPU9250_SIG_PATH_RESET, 3)

    _gyro_range = RWBits(2, _MPU9250_GYRO_CONFIG, 3)
    _accel_range = RWBits(2, _MPU9250_ACCEL_CONFIG, 3)

    _filter_bandwidth = RWBits(2, _MPU9250_CONFIG, 3)

    _raw_accel_data = StructArray(_MPU9250_ACCEL_OUT, ">h", 3)
    _raw_gyro_data = StructArray(_MPU9250_GYRO_OUT, ">h", 3)
    _raw_temp_data = ROUnaryStruct(_MPU9250_TEMP_OUT, ">h")

    _cycle = RWBit(_MPU9250_PWR_MGMT_1, 5)
    _cycle_rate = RWBits(2, _MPU9250_PWR_MGMT_2, 6, 1)

    sleep = RWBit(_MPU9250_PWR_MGMT_1, 6, 1)
    ...
    sample_rate_divisor = UnaryStruct(_MPU9250_SMPLRT_DIV, ">B")
